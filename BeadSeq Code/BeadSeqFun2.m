...
%        puckimage(:,:,k)=puckimagefull(ROI(1,1):ROI(1,2),ROI(2,1):ROI(2,2),k);
    ...
    if m==calibratechannels(1) && calibrate %Use this to look at the mixing between Cy3 and TxR after correction
        figure(7)
        tmp1=reshape(puckzscores(:,:,2),ROIHeight*ROIWidth,1);
        perm=randperm(length(tmp1));
        tmp1=tmp1(perm(1:10000));
        tmp2=reshape(puckzscores(:,:,3),ROIHeight*ROIWidth,1);
        tmp2=tmp2(perm(1:10000));
        scatter(tmp1,tmp2)
    end
    ...
    if mod(m,2)==1
        ...
        if m==calibratechannels(2) && calibrate %this is one way to look at phasing: compare the mixing between 488 and 647 in round 1 and round 2. Then compare with the background subtracted    
            figure(10)
            tmp1=reshape(PreviousRoundZScores(:,:,1),ROIHeight*ROIWidth,1);
            perm=randperm(length(tmp1));
            tmp1=tmp1(perm(1:10000));
            tmp2=reshape(puckzscores(:,:,1),ROIHeight*ROIWidth,1);
            tmp2=tmp2(perm(1:10000));
            scatter(tmp1,tmp2)
        end        
    ...
    if m==calibratechannels(2) && calibrate %this is one way to look at phasing: compare the mixing between 488 and 647 in round 1 and round 2. Then compare with the background subtracted
        figure(6)
        tmp1=reshape(puckzscores(:,:,4),ROIHeight*ROIWidth,1);
        perm=randperm(length(tmp1));
        tmp1=tmp1(perm(1:10000));
        tmp2=reshape(puckzscores(:,:,1),ROIHeight*ROIWidth,1);
        tmp2=tmp2(perm(1:10000));
    ...
    if EnforceBaseBalance && BarcodeSequence(m)~=0
        multiplier=[1 1 1 1];
        TestImage=puckzscores((round(ROIHeight/2)-500):(round(ROIHeight/2)+500),(round(ROIWidth/2)-500):(round(ROIWidth/2)+500),:);
        while true
            Counter=Counter+1;
            if Counter>1000
            ...
            for j=1:4
                tmppuckzscores(:,:,j)=multiplier(j)*TestImage(:,:,j);
            end
            [TestM,TestI]=max(tmppuckzscores,[],3); %I is the matrix of indices.    
            TestI(TestM<ZScoreThreshold)=0; %This is the z score threshold -- z scores less than ZScoreThreshold are treated as 0.
            for j=1:4
        ...
        for j=1:4
            puckzscores(:,:,j)=multiplier(j)*puckzscores(:,:,j);
        end
    end
