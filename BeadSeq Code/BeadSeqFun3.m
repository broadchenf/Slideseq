function [Bead BeadImage]=BeadSeqFun3(BaseName, suffix, OutputFolder,BeadZeroThreshold,BarcodeSequence,NumPar,NumLigations,PuckName)
...
%For the confocal 150 is usually good. Thanks, Marvin.
    starttime=tic();
backgroundthreshold=[200, 150, 200, 150]; %However, this can break if there is too much background in any one image
...
%        puckimage(:,:,k)=puckimagefull(ROI(1,1):ROI(1,2),ROI(2,1):ROI(2,2),k);
    ...
    AdjustmentWeightsForThisBase=[1,1,1,1];    
    while 1
%        disp('Base Balance Not Satisfied. Adjusting Weights')
        for kl=1:4
            puckzscorestmp(:,:,kl)=puckzscores(:,:,kl)*AdjustmentWeightsForThisBase(kl);
        end
        [M,I]=max(puckzscorestmp,[],3); %I is the matrix of indices.
        I(M<ZScoreThreshold)=0; %This is the z score threshold -- z scores less than ZScoreThreshold are treated as 0.
        basebalance=[nnz(I==1),nnz(I==2),nnz(I==3),nnz(I==4)];
        if max(basebalance)-min(basebalance)>0.15*min(basebalance)
            [minval,indx]=min(basebalance);
            AdjustmentWeightsForThisBase(indx)=AdjustmentWeightsForThisBase(indx)+0.1;
        else
            break
        end
        %We now implement the adaptive basecalling.
        
        
    end
    Indices(:,:,m)=uint8(I);
    AdjustmentWeights(:,m)=AdjustmentWeightsForThisBase;    
...
figure(32) %analysis of the AdjustmentWeights
b=bar(AdjustmentWeights');
b(1).FaceColor='b';
b(2).FaceColor='g';
b(3).FaceColor='y';
b(4).FaceColor='r';
title('AdjustmentWeights per ligation');
export_fig([OutputFolder,'Report_BaseCalling_',PuckName,'.pdf'],'-append');
%export_fig([OutputFolder,'Report_BaseCalling.pdf'],'-append');
