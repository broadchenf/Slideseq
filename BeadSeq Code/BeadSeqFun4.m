function [Bead BeadImage]=BeadSeqFun2(BaseName, suffix, OutputFolder,BeadZeroThreshold,BarcodeSequence,NumPar,NumLigations,PuckName,varargin)
...
%There is only one optional variable, which is BalanceZScores. It is
%optional for legacy purposes. The default is 1.
if isempty(varargin)
    BalanceZScores=1;
else
    BalanceZScores=varargin{1};
end

starttime=tic();
backgroundthreshold=[350, 150, 200, 150]; %However, this can break if there is too much background in any one image
...
%        puckimage(:,:,k)=puckimagefull(ROI(1,1):ROI(1,2),ROI(2,1):ROI(2,2),k);
        ...
        %We now find the minimum in the pixel value distribution
        [N,edges]=histcounts(pixelvalsabovethreshold,1:100:max(pixelvalsabovethreshold));
        minima=find(N(2:(end-1))<N(1:(end-2)) & N(2:(end-1))<N(3:(end)));
        cutoff=edges(minima(1)+1);
        pixelvalsabovethreshold=pixelvals(pixelvals(:,k)>cutoff,k);

        %We now find the first maximum after the minimum, which is basically the dark bead peak
        [N,edges]=histcounts(pixelvalsabovethreshold,1:100:max(pixelvalsabovethreshold));
        maxima=find(N(2:(end-1))>N(1:(end-2)) & N(2:(end-1))>N(3:(end)));
        peak=edges(maxima(1)+1);
        DarkBeadDistribution=pixelvals(pixelvals(:,k)>cutoff & pixelvals(:,k)<(cutoff+2*(peak-cutoff)),k);
        
        %NOTE: WE SHOULD OUTPUT THE HISTOGRAMS HERE
        ...
        pixelvalsmidMean(k)=mean(DarkBeadDistribution);
        pixelvalsmidStd(k)=std(DarkBeadDistribution);    
        puckzscores(:,:,k)=(double(puckimage(:,:,k))-pixelvalsmidMean(k))/pixelvalsmidStd(k);
        
        figure(36)
        subplot(2,2,subfignum)
        histogram(DarkBeadDistribution,1:100:3000)
        set(gca,'yscale','log')
        figure(37)
        subplot(2,2,subfignum)
        histogram(pixelvals(:,k),1:100:3000)
...
%    for k=1:4
%        tmp=puckzscores(:,:,k);
%        meanzscores(k)=mean(puckzscores(puckimage(:,:,k)>pixelvalsmidMean(k)));
%    end
    meanzscores=mean(mean(puckzscores,2),1);


    if BalanceZScores
        for k=1:4
            puckzscores(:,:,k)=puckzscores(:,:,k)*mean(meanzscores)/meanzscores(k);
        end    
    end
    ...
    puckzscores(:,:,3)=TxRHelpFactor*(puckzscores(:,:,3)-Cy3TxRMixing*(puckzscores(:,:,2)>2).*puckzscores(:,:,2)); %We subtract Cy3 from TxR, only if the Cy3 signal is significant, and then we multiply by the TxR help factor.
...
%We often lose a lot of beads because of few-pixel displacements in the image
%registration
disp('Beginning final registration phase')
%We choose a very small region corresponding to 500x500 pixels in the
%middle of the image
TestStack=Indices((round(ROIHeight/2)-250):(round(ROIHeight/2)+250),(round(ROIWidth/2)-250):(round(ROIWidth/2)+250),:);
...
fprintf(fileid,['The total runtime for basecalling was ',num2str(toc(starttime)/60),' minutes.\n',...
    ...
    'BalanceZScores=',num2str(BalanceZScores),'.\n',...
