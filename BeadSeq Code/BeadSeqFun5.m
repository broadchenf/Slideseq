...
starttime=tic();
backgroundthreshold=[600 600 600 600];%[350, 150, 200, 150]; %However, this can break if there is too much background in any one image
...
%        puckimage(:,:,k)=puckimagefull(ROI(1,1):ROI(1,2),ROI(2,1):ROI(2,2),k);
        ...
        pixelvalsmidStd(k)=std(pixelvalsabovethreshold(floor(0.66*length(pixelvalsabovethreshold)):end));    
