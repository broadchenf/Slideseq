...
%    m
    ...
    for k=1:4
        puckimage(:,:,k)=imread([BaseName,pad(num2str(m),2,'left','0'),' channel ',int2str(k),suffix,' transformLMC.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)});
    end
...
for dropbase=0:14
    ...
    if length(manypixelbarcodestmp)>maxbarcodes
        if dropbase==0
            maxbarcodes=length(manypixelbarcodestmp)+4000; %We prefer to keep all the ligations
        else
