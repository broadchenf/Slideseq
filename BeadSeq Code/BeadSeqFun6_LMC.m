function [Bead BeadImage]=BeadSeqFun2(BaseName, suffix, OutputFolder,BeadZeroThreshold,BarcodeSequence,NumPar,NumLigations,PuckName,EnforceBaseBalance,BaseBalanceTolerance,varargin)
...
%    m
    ...
    if mod(m,2)==1
        ...
        for k=1:4 %this is ugly, but I couldn't work out a better way to do it
            puckimage(:,:,k)=puckimage(:,:,k)-SubtractLastRound.*PreviousRoundMixing.*(PreviousRoundImage(:,:,k));
        end        
    end
...
%    OutputFolder='C:\Users\Sam\Dropbox (MIT)\Project - SlideSeq\BeadSeq Code\find_roi\InputFolder-Puck85-170818\Position A1 Base Calls - Params 7\';
    ...
    imwrite(256*uint16(Indices(:,:,1)==1),[OutputFolder,'Channel1CallsLMC.tiff']);
    imwrite(256*uint16(Indices(:,:,1)==2),[OutputFolder,'Channel2CallsLMC.tiff']);
    imwrite(256*uint16(Indices(:,:,1)==3),[OutputFolder,'Channel3CallsLMC.tiff']);
    imwrite(256*uint16(Indices(:,:,1)==4),[OutputFolder,'Channel4CallsLMC.tiff']);
    for jm = 2:size(Indices,3)
        imwrite(256*uint16(Indices(:,:,jm)==1),[OutputFolder,'Channel1CallsLMC.tiff'],'WriteMode','append');
        imwrite(256*uint16(Indices(:,:,jm)==2),[OutputFolder,'Channel2CallsLMC.tiff'],'WriteMode','append');
        imwrite(256*uint16(Indices(:,:,jm)==3),[OutputFolder,'Channel3CallsLMC.tiff'],'WriteMode','append');
        imwrite(256*uint16(Indices(:,:,jm)==4),[OutputFolder,'Channel4CallsLMC.tiff'],'WriteMode','append');
    end

    if 0 %this is currently not executed because the maximum tiff file size is exceeded
    imwrite(imread([BaseName,pad(num2str(1),2,'left','0'),' channel ',int2str(1),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel1ImageLMC.tiff']);
    imwrite(imread([BaseName,pad(num2str(1),2,'left','0'),' channel ',int2str(2),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel2ImageLMC.tiff']);
    imwrite(imread([BaseName,pad(num2str(1),2,'left','0'),' channel ',int2str(3),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel3ImageLMC.tiff']);
    imwrite(imread([BaseName,pad(num2str(1),2,'left','0'),' channel ',int2str(4),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel4ImageLMC.tiff']);
    pause(2);
    for jm = 2:size(Indices,3)
        imwrite(imread([BaseName,pad(num2str(jm),2,'left','0'),' channel ',int2str(1),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel1ImageLMC.tiff'],'WriteMode','append');
        imwrite(imread([BaseName,pad(num2str(jm),2,'left','0'),' channel ',int2str(2),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel2ImageLMC.tiff'],'WriteMode','append');
        imwrite(imread([BaseName,pad(num2str(jm),2,'left','0'),' channel ',int2str(3),suffix,' transform.tif'],'PixelRegion',{ROI(1,1:2),ROI(2,1:2)}),[OutputFolder,'Channel3ImageLMC.tiff'],'WriteMode','append');
...
export_fig([OutputFolder,'Report_BaseCalling_',PuckName,'LMC.pdf'],'-append');    %export_fig([OutputFolder,'Report.pdf'],'-append');
...
export_fig([OutputFolder,'Report_BaseCalling_',PuckName,'LMC.pdf'],'-append');
...
parfor parnum=1:NumPar
    ...
    imwrite(LocalBeadImage,[OutputFolder,'Worker_',num2str(parnum),'_LocalBeadImageLMC.tif']);
...
Bead=struct('Barcodes',num2cell(BeadBarcodes),'Locations',num2cell(BeadLocations,1),'Pixels',BeadPixCelljoined);
    ...
    imwrite(BeadImage,[OutputFolder,'BeadImageLMC.tif']);
...
fileid=fopen([OutputFolder,'MetricsLMC.txt'],'w');
