...
ExpressionGenes={};
VarianceGenes={};
for clus=1:length(ClusterToAnalyze)
if FilterGenes==1 || FilterGenes==3
    ...
    GoodGenes=table2array(ExpressionTable(:,ClusterToAnalyze(clus)+1))>1e-1;
    GoodGeneList=ExpressionTable.GeneNames(GoodGenes);
    if FilterGenes==3
        ExpressionGenes=union(ExpressionGenes,GoodGeneList);
    end
end
if FilterGenes==2 || FilterGenes==3
    ...
    GoodBeads=find(table2array(VarianceTable(:,ClusterToAnalyze(clus)+1))>0 & table2array(ExpressionTable(:,ClusterToAnalyze(clus)+1))>0); 
    GoodGenes=table2array(VarianceTable(GoodBeads,ClusterToAnalyze(clus)+1))./(table2array(ExpressionTable(GoodBeads,ClusterToAnalyze(clus)+1)).^2)>7.5 & table2array(ExpressionTable(GoodBeads,ClusterToAnalyze(clus)+1))>1e-2;
    GoodGeneList=VarianceTable.GeneNames(GoodBeads(GoodGenes));
    if FilterGenes==3
        VarianceGenes=union(VarianceGenes,GoodGeneList);
    end
