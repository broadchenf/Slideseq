...
nKeypointsThresh = 10; %Minimum number of keypoint matches. Default was 50. We use a very low threshold because the match is essentially always on tile 1.
...
BaseName='InputFolder-Puck3-170701\Puck 3 Ligation ';
...
for k=1:channelnum
    mapstack(:,:,k) = im2single(imadjust(imread([BaseName,int2str(1),'.tif'],'index',k)));
    mapOrigstack(:,:,k)=imread([BaseName,int2str(1),'.tif'],'index',k);
end
dims=size(mapstack);
...
for k=1:channelnum
    imwrite(mapOrigstack(:,:,k),[BaseName,'1',' channel ',int2str(k),' transform.tif'])
end
...
%    map = imadjust(im2single(mapOrig));
    ...
    for k=1:channelnum
        querystack(:,:,k) = im2single(imadjust(imread([BaseName,int2str(mm),'.tif'],'index',k)));
        queryOrigstack(:,:,k)=imread([BaseName,int2str(mm),'.tif'],'index',k);
    end
...
%    end
    for k=1:channelnum
        imwrite(imtransform(queryOrigstack(:,:,k),tform,'XData',[1, size(query,2)],'YData',[1, size(query,1)]),[BaseName,int2str(mm),' channel ',int2str(k),' transform.tif'])
    end
