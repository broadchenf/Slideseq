...
FolderWithRawTiffs='D:\Slideseq\Raw\180626 - Pucks 180620-X\';
...
for puck=1:length(PuckNames)
    mkdir([OutputFolders{puck},'BeadMapping_',mappingstarttimereadable]);
    MappingOutputFolders{puck}=[OutputFolders{puck},'BeadMapping_',mappingstarttimereadable,'\'];
...
%    [UniqueMappedBeads(:).IlluminaBarcode]=deal(IlluminaBarcodesUnique([UniqueMappedBeads.IndexofIlluminaBarcode]));
    ...
    disp(['For puck ',PuckNames{puck},' there were ',num2str(length(UniqueMappedBeads)),' barcodes.'])
