...
BeadType="180402 and 180728 on same sled"; %for 14bp barcodes from 180402 beads
...
RunAnalogizer=zeros(30,1);%[0,2,2,0,2,0,2,2,2,2,2,2,2,0,0,0,1,1,1,0,2,2,2,2];
...
NumLigations=4; %We assume that the missing ligation is Truseq-4 if you are doing 13.
NumBases=4; %This is the number of bases sequenced
BarcodeSequence=[1,2,3,4];
...
FolderWithRawTiffs='D:\Slideseq\Raw\181003 - Pucks 181003Epi-X\'; 
FolderWithProcessedTiffs='D:\Slideseq\Processed\';
tmpfolder='D:\pucktmp\';
IndexFiles={'primers truseq and up onlytestepi'}; %give the prefixes of each of the files. The next character after should be 't',
LigationToIndexFileMapping=[1,1,1,1];%for ligations 1:20, which nd2 file number are the ligations found in?
...
tnumMapping=[1,2,3,4];
PucksToAnalyze=[1,2,3,4]; %These are the values of xy from the .nd2 file that will be analyzed.
%Note that the order in PuckNames should match the order in the .nd2 file.
PuckNames={'Puck_181003EpiRv2_1','Puck_181003EpiRv2_2','Puck_181003EpiRv2_3','Puck_181003EpiRv2_4'}; %give the names of the pucks
...
XCorrBounds=[2900,3100,2900,3100]; %This is the ROI used in channel registration
RegisterColorChannels=1;
BeadZeroThreshold=1;
PixelCutoffRegistration=400;
PixelCutoffBasecalling=400;
...
for pucknum=1:length(PuckNames)
    ...
    for ligation=1:NumLigations
        ...
        elseif exist([FolderWithRawTiffs,IndexFiles{LigationToIndexFileMapping(ligation)},'t',num2str(tnum),'xy',num2str(puck),'.tiff'],'file')
            filename=[FolderWithRawTiffs,IndexFiles{LigationToIndexFileMapping(ligation)},'t',num2str(tnum),'xy',num2str(puck),'.tiff'];
        ...
        elseif exist([FolderWithRawTiffs,IndexFiles{LigationToIndexFileMapping(ligation)},'xy',pad(num2str(puck),2,'left','0'),'.tif'],'file')
            filename=[FolderWithRawTiffs,IndexFiles{LigationToIndexFileMapping(ligation)},'xy',pad(num2str(puck),2,'left','0'),'.tif'];
        else
            assert(1==0) 
        end
...
for puck=1:length(PuckNames) %note we are trying to run this overnight without parfor, because we can't figure out why it keeps crashing, and it only crashes for large sized images, and only on puck 8_7
    ...
    Suffix='_Stitched';
    find_roi_stack_fun(BaseName,Suffix,ImageSize,'PixelCutoff',PixelCutoffRegistration,'XCorrBounds',XCorrBounds,'RegisterColoras',1);
...
%The illumina DGE should have the puck date and number, i.e. 180819_3,
%followed by an underscore, so 180819_3_
%The underscore is to distinguish 180819_2_ from 180819_23, for example
...
for puck=1:length(PuckNames)
    ...
    call is of the form:
    python3 autoNMFreg_mac.py -da /Volumes/broad_macosko/data/NMFreg/data -dp /Users/rstickel/Documents/mac_lab/slideseq/data/seq_data/puck_seq/Puck_180819_10/BeadMapping_8-23_0014 -t hippocampus -c 100 -dge MappedDGEForR -bl BeadLocationsForR
    we then have to conform 
    We also have to retrieve the mean and variance by gene from somewhere
