...
FolderWithRawTiffs='D:\Slideseq\Raw\181003 - Pucks 181003Con-X\';
FolderWithProcessedTiffs='D:\Slideseq\Processed\';
tmpfolder='D:\pucktmp\';
IndexFiles={'primers truseq and up onlytest'}; %give the prefixes of each of the files. The next character after should be 't',
LigationToIndexFileMapping=[1,1,1,1];%for ligations 1:20, which file number are the ligations found in?
...
PuckNames={'Puck_181003Epi_1','Puck_181003Epi_2','Puck_181003Epi_3','Puck_181003Epi_4'}; %give the names of the pucks
...
for puck=1:length(PuckNames) %note we are trying to run this overnight without parfor, because we can't figure out why it keeps crashing, and it only crashes for large sized images, and only on puck 8_7
    ...
    Suffix='_Stitched';
    find_roi_stack_funSpec(BaseName,Suffix,ImageSize,'PixelCutoff',PixelCutoffRegistration,'XCorrBounds',XCorrBounds,'RegisterColoras',1);
