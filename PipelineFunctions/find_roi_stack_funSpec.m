function find_roi_stack_funSpec(BaseName,suffix,ImageSize,varargin)
...
nRansacTrials = 5000; %2000 by default. Increase for more reliable matching
...
for k=1:channelnum
    mapOrigstack(:,:,k)=imread([BaseName,pad(num2str(1),2,'left','0'),suffix,'.tif'],'index',k+4,'PixelRegion',{[1,ImageSize],[1,ImageSize]});
end
...
for mm=2:l
    ...
    for k=1:channelnum
        queryOrigstack(:,:,k,mm)=imread([BaseName,pad(num2str(mm),2,'left','0'),suffix,'.tif'],'index',k+4,'PixelRegion',{[1,ImageSize],[1,ImageSize]});
    end
